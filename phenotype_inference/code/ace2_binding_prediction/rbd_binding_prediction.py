...
parser.add_argument('--data', type=str, default='../../external_validation_dataset/ace2_binding_cao.csv')
parser.add_argument('--out_path', type=str, default='prediction/ace2_binding_cao_pred.csv', help="Output CSV path")
...
def main():
    ...
    if len(all_preds) != len(df):
        print(f"Warning: Pred len {len(all_preds)} != DF len {len(df)}")
    
    df['pred_binding'] = all_preds
